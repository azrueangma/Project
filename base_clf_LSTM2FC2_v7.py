...
SAVE_DIR = "./output/v7_output/"
SAMPLING_FILE_PATH = "./data/sampling1.csv"
BOARD_PATH = './board/v7_board'
    ...
    for atk_dir in atk_dirs:
        ...
        if temp in test_sample:
            ...
            for beam_dir in beam_dirs:
                ...
                #trainX.extend(pdw_list[:ntrain])
                trainX.extend(random.sample(pdw_list[:ntrain], ntrain//10))
                validationX.extend(random.sample(pdw_list[ntrain:ntrain+nvalidation], nvalidation//10))
                test1X.extend(random.sample(pdw_list[ntrain+nvalidation:],ntest1//10))
...
class Model:
    ...
    def _build_model(self):
        ...
        with tf.device("/gpu:{}".format(NUM_GPU_DEVICE)):
...
with tf.device('/cpu:0'):
    ...
    with tf.variable_scope("TrainQueue"):
        ...
        batch_queue = prefetch_queue.prefetch_queue([t_batchX, t_batchY1], capacity = 60, num_threads = 15, dynamic_pad=True)
